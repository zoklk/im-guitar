...
push(unique_ptr<Product>)             // slots_[0]에 적재. 막혀있으면 onOverflow(p)
update(tick):
  1. 출구 방출: slots_[length-1] != null && downstream != null
     → downstream.acceptProduct(move(slots_[length-1]))
  2. 응축 시프트: i = length-1 → 1 역방향 순회
     - if slots_[i] == null && slots_[i-1] != null:
         slots_[i] = move(slots_[i-1])     // 빈 칸이 출구쪽으로 못 새도록 끌어당김
onOverflow(unique_ptr<Product> p):
  - mode == Drop          → dropAndLog(move(p))
  - mode == Backpressure  → publishBackpressure() + p 소멸 (제품은 어쨌든 사라짐)
dropAndLog(unique_ptr<Product> p):
  - statistics_.lost++
...
`Machine.acceptProduct(unique_ptr<Product>)` 가상 메서드 호출로 일원화. 기본 구현은 단일 `inputBuffer_`에 push, Assembler/Collector는 종류별 typedBuffer에 분류 (Phase 3에서 override). Conveyor는 ProductType을 모름.
...
- 하위: Machine* downstream 보유 → Phase 3 forward declaration

## 테스트

`tests/phase_2_conveyor.cpp`:

- 빈 컨베이어에 push → 입구 슬롯 점유. 시프트 동작 — N틱 후 출구 도달
- 출구 슬롯 product + downstream 정상 → downstream.acceptProduct 호출 mock 검증, 슬롯 reset
- 가득찬 상태에서 push (Drop 모드) → statistics.lost++ + wip--, eventLog.appendDirect 호출
- 가득찬 상태에서 push (Backpressure 모드) → broker.publish 호출, statistics.lost 미증가 (lost는 Drop 전용)
...
- Conveyor → Machine acceptProduct 시그니처 확정은 Phase 3에서 (forward declare로 시작)

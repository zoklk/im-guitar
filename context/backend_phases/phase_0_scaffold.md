...
- **추가**: `EventType` (`Fault`, `Resume`, `Started`, `Completed`, `Backpressure`)
- **추가**: `OverflowMode` (`Drop`, `Backpressure`) — Conveyor가 시나리오별 동작 분기에 사용
...
```
struct Event              { EventType type; std::string sourceId; int tick; void* payload; };
...
- `payload` 타입은 `void*`로 시작. 페이로드 종류 늘면 Phase 5에서 `std::variant` 전환 검토
...
- `MachineSnap`: `type` 추가, `inputBuffer` / `currentProduct` 추가 (vector<ProductSnap>), `assignedTechId: optional<string>` 추가 (snap 생성 시 Technician 목록 훑어 채우는 derive 필드)
